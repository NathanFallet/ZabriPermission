...
public class Cmd implements CommandExecutor {
		...
		if (args.length != 0) {
			if (args[0].equalsIgnoreCase("list")) {
				...
				if (args.length == 2) {
					ZabriGroup g = ZabriPermission.getInstance().getGroup(args[1]);
					if (g != null) {
						sender.sendMessage("§6-- Groupe : " + g.getName() + " --\n" + "§ePrefixe : §r" + g.getPrefix()
								+ "§r\n" + "§eSuffixe : §r" + g.getSuffix() + "§r\n" + "§ePermissions : "
								+ g.getPermissions().toString() + "\n" + "§eHéritances : "
								+ g.getInherits().toString());
					} else {
						sender.sendMessage("§cCe groupe n'existe pas !");
					}
				} else {
	...
	public void sendHelp(CommandSender sender, String label) {
		sender.sendMessage("§e/" + label + " list : Lister les groupes\n" + "§e/" + label
				+ " add <nom du groupe> : Ajouter un groupe\n" + "§e/" + label + " del <groupe> : Supprimer un groupe\n"
				+ "§e/" + label + " info <groupe> : Afficher les informations d'un groupe\n" + "§e/" + label
				+ " prefix <groupe> <prefixe> : Changer le prefix d'un groupe\n" + "§e/" + label
				+ " suffix <groupe> <suffixe> : Changer le suffixe d'un groupe\n" + "§e/" + label
				+ " addp <groupe> <permission> : Ajouter une permission à un groupe\n" + "§e/" + label
				+ " delp <groupe> <permission> : Supprimer une permission à un groupe\n" + "§e/" + label
				+ " addi <groupe> <héritance> : Ajouter une héritance à un groupe\n" + "§e/" + label
				+ " deli <groupe> <héritance> : Supprimer une héritance à un groupe\n" + "§e/" + label
				+ " player <pseudo> <groupe> : Changer le groupe d'un joueur\n" + "§e/" + label
				+ " playeraddp <pseudo> <permission> : Ajouter une permission à un joueur\n" + "§e/" + label
				+ " playerdelp <pseudo> <permission> : Supprimer une permission à un joueur\n");
	}
